    ...
    <ChartCard title="Origem por Canal">
      <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
        <ResponsiveContainer width={100} height={100}>
          <PieChart>
            <Pie data={d.canais} dataKey="value" innerRadius={26} outerRadius={46} paddingAngle={3} startAngle={90} endAngle={450}>
              {d.canais.map((c, i) => <Cell key={i} fill={c.color} />)}
            </Pie>
            <Tooltip {...tt} />
          </PieChart>
        </ResponsiveContainer>
  ...
  if (blockId === 'table-leads') return (
    <ChartCard title="Leads Recentes" col2>
      <table style={{ width: '100%', borderCollapse: 'collapse' }}>
        ...
        <tbody>
          {l.recentes.map((r, i) => (
            <tr key={i} style={{ borderBottom: i < l.recentes.length - 1 ? '1px solid var(--br)' : 'none' }}>
  ...
  if (blockId === 'table-compras') return (
    <ChartCard title="Compras Recentes" col2>
      <table style={{ width: '100%', borderCollapse: 'collapse' }}>
        ...
        <tbody>
          {d.recentes.map((r, i) => (
            <tr key={i} style={{ borderBottom: i < d.recentes.length - 1 ? '1px solid var(--br)' : 'none' }}>
              <td style={{ padding: '9px 10px', fontSize: 11.5, color: 'var(--t1)' }}>{r.nome}</td>
              <td style={{ padding: '9px 10px', fontSize: 11, color: 'var(--t2)' }}>{r.origem}</td>
              <td style={{ padding: '9px 10px', fontSize: 12, fontWeight: 700, color: '#10B981' }}>R${r.valor.toFixed(2)}</td>
              <td style={{ padding: '9px 10px', fontSize: 11, color: 'var(--t3)' }}>{r.data}</td>
            </tr>
          ))}
        </tbody>
...
export default function PersonalizadoTemplate({ clienteId, initialBlocks }: Props) {
