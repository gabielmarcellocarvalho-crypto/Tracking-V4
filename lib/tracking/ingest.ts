...
export async function ingerirEvento(
  ...
  for (const conv of conversoes) {
    ...
    // Envio server-side imediato (Meta CAPI / Google Enhanced) — se não
    // houver credenciais configuradas ainda, a conversão fica como
    // "aguardando-conexao" e é reprocessada depois pelo cron de reenvio.
    ...
    } else if (conv.plataforma === 'google-enhanced') {
      try {
        await enviarConversaoParaGoogle(clienteId, conversaoId)
      } catch (err) {
        console.error('[ingest] falha no envio imediato ao Google Ads Enhanced:', err)
      }
    }
  }
